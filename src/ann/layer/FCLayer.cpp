...
{
    //YOUR CODE IS HERE
    //if (this->m_trainable) this->m_aCached_X = X; //n n_in
    this->m_aCached_X = X;
    auto Y = xt::linalg::dot(X, xt::transpose(m_aWeights)); //n_out n
    if (m_bUse_Bias) Y += m_aBias;
    return  Y;
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) 
{
    ...
    m_aGrad_W =  xt::mean(outer_stack(DY, m_aCached_X));
    
    if (m_bUse_Bias) m_aGrad_b = xt::sum(DY);
    //2x50 50x2

    auto DX = xt::linalg::dot(DY, m_aGrad_W);
    return DX;

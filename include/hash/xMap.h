                ...
                void (*deleteKeys)(xMap<K,V>* pMap) )
{
    ...
    {
        table[i] = DLinkedList<Entry*>(&DLinkedList<Entry*>::free, nullptr); 
    }
}
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    for (auto pEntry : list) 
    {
        if (((keyEqual == nullptr || keyEqual(pEntry->key, key)) && 
            (valueEqual == nullptr || valueEqual(pEntry->value, value)))||(keyEQ(pEntry->key,key) && valueEQ(pEntry->value,value))) 
        {
            if (deleteKeyInMap != nullptr) deleteKeyInMap(pEntry->key);
...
template<class K, class V> //ok
bool xMap<K,V>::            containsKey(K key)
{
    //YOUR CODE IS HERE 
    for(int idx=0; idx < this->capacity; idx++)
    {
        DLinkedList<Entry*>& pList = this->table[idx];
        for(auto pEntry: pList)
        {
            if ((keyEqual != nullptr && keyEqual(pEntry->key, key)) || keyEQ(pEntry->key, key)) 
...
bool xMap<K,V>::            containsValue(V value)
{
    ...
    for(int idx=0; idx < this->capacity; idx++)
    {
        ...
        for(auto pEntry: pList)
        {
            if ((valueEqual != nullptr && valueEqual(pEntry->value, value)) || valueEQ(pEntry->value, value))
            {
                return true;
            }
        }

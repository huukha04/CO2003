...
void Heap<T>::remove(T item, void (*removeItemData)(T))
{
    ...
    count--;
    reheapDown(index);
    if (removeItemData!= nullptr) removeItemData(item);
...
void Heap<T>::reheapUp(int position)
{
    ...
    while (position > 0 && (comparator == nullptr ? 
    elements[position] < elements[parent] : 
    comparator(elements[position], elements[parent]) < 0)) 
    {
...
void Heap<T>::reheapDown(int position)
{
    ...
    // Kiểm tra con trái
    if (left < count && (comparator == nullptr ? elements[left] < elements[selected]
            : comparator(elements[left], elements[selected]) < 0)) 
            {
        selected = left;
    }

    // Kiểm tra con phải
    if (right < count && (comparator == nullptr ? elements[right] < elements[selected]
            : comparator(elements[right], elements[selected]) < 0)) 
            {
        selected = right;
    }
